	...
	<article id="comment-<?php comment_ID(); ?>" class="comment">
		<header class="comment-meta comment-author vcard">
			...
			printf( '<cite><b class="fn">%1$s</b> %2$s</cite>',
				...
				( $comment->user_id === $post->post_author ) ? '<span>' . esc_html__( 'Post author', 'my-theme' ) . '</span>' : ''
			); ?>
			<!--Rating-->
			<?php if(!empty($rate)) : ?>
			...
			<?php printf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
				...
				sprintf( esc_html__( '%1$s at %2$s', 'my-theme' ), get_comment_date(), get_comment_time() )
			);
			?>
		</header><!-- .comment-meta -->
		...
		<section class="comment-content comment">
			<?php comment_text(); ?>
			<?php edit_comment_link( esc_html__( 'Edit', 'my-theme' ), '<p class="edit-link">', '</p>' ); ?>
